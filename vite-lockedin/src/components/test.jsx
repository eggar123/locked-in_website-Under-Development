...
import { Image, ScrollControls, Scroll, useScroll, Gltf } from '@react-three/drei'
...
function Minimap() {
  ...
  return (
    <group ref={ref}>
      {urls.map((_, i) => (
        <line key={i} geometry={geometry} material={material} position={[i * 0.06 - urls.length * 0.03, -height / 1.5 + 0.6, 0]} />
      ))}
    </group>
  )
}
...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
  ...
  return <Gltf position={position} scale={scale} onClick={click} onPointerOver={over} onPointerOut={out} />
}
